...
class ShippingCompanyAnalytics:
	...
	def get_report_summary(self):
		...
		city_stats = {}
		for d in self.data:
			city = d.get("city") or _("Not Set")
			if city not in city_stats:
				city_stats[city] = {"count": 0, "total_cod": 0}
			city_stats[city]["count"] += 1
			city_stats[city]["total_cod"] += flt(d.cod_amount)

		unique_cities = len(city_stats)
		total_cod_by_city = sum(stats["total_cod"] for stats in city_stats.values())
		...
		self.report_summary = [
			...
			{
				"value": total_cod_by_city,
				"indicator": "Green",
				"label": _("Total COD by City"),
				"datatype": "Currency",
			},
		]
